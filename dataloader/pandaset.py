...
class LoadPandasetInfo:
    def __init__(self, config, data_path, imageset='train'):
        ...
        dataset_yaml = yaml.safe_load(
            open(config['dataset_params']['label_mapping']))
        if imageset == 'train':
            self.data_path = [os.path.join(data_path, x)
                              for x in dataset_yaml['train_list']]
        else:
            self.data_path = [os.path.join(data_path, x)
                              for x in dataset_yaml['val_list']]
        self.learning_map = dataset_yaml['learning_map']
        self.camera_list = ['back_camera', 'front_camera', 'front_left_camera',
    ...
    def get_available_scenes(self):
        ...
        for data_dir in self.data_path:
            ...
            for path in lidar_paths[:1]:
                if 'pkl.gz' not in path:
                    continue
                self.ava_scenes.extend([data_dir, path.split('.pkl.gz')[
                                       0], x] for x in self.camera_list)

    def get_data_info(self, index):
        ava_scenes_idx = self.ava_scenes[index]
        lidar_data = pickle.load(gzip.open(os.path.join(
            ava_scenes_idx[0], 'lidar', ava_scenes_idx[1]+'.pkl.gz'))).values
        semseg_data = pickle.load(gzip.open(os.path.join(
            ava_scenes_idx[0], 'annotations', 'semseg', ava_scenes_idx[1]+'.pkl.gz'))).values
        cuboids_data = pickle.load(gzip.open(os.path.join(
            ava_scenes_idx[0], 'annotations', 'cuboids', ava_scenes_idx[1]+'.pkl.gz'))).values
        camera_data = Image.open(os.path.join(
            ava_scenes_idx[0], 'camera', ava_scenes_idx[2], ava_scenes_idx[1]+'.jpg'))
        camera_pose = json.load(open(os.path.join(
            ava_scenes_idx[0], 'camera', ava_scenes_idx[2], 'poses.json')))[int(ava_scenes_idx[1])]
        lidar_pose = json.load(open(os.path.join(ava_scenes_idx[0], 'lidar', 'poses.json')))[
            int(ava_scenes_idx[1])]
        camera_intrinsics = json.load(open(os.path.join(
            ava_scenes_idx[0], 'camera', ava_scenes_idx[2], 'intrinsics.json')))
        ...
        camera_semseg_data = np.vectorize(
            self.learning_map.__getitem__)(ori_camera_semseg_data)
        data_dict.update({
...
class PandasetDateset(data.Dataset):
    def __init__(self, config, loader_config, data_path, imageset='train'):
        ...
        self.image_normalizer = config['dataset_params'].get(
    ...
    def __getitem__(self, index):
        ...
        lidar2ego_xyz = geometry.lidar_points_to_ego(
            data_dict['xyz'], data_dict['calib_info']['lidar_pose'])
        ref_pc = lidar2ego_xyz.copy()
        ref_labels = data_dict['labels'].copy()
        ref_index = np.arange(len(ref_pc))

        mask_x = np.logical_and(
            lidar2ego_xyz[:, 0] > self.min_volume_space[0], lidar2ego_xyz[:, 0] < self.max_volume_space[0])
        mask_y = np.logical_and(
            lidar2ego_xyz[:, 1] > self.min_volume_space[1], lidar2ego_xyz[:, 1] < self.max_volume_space[1])
        mask_z = np.logical_and(
            lidar2ego_xyz[:, 2] > self.min_volume_space[2], lidar2ego_xyz[:, 2] < self.max_volume_space[2])
        ...
        if self.loader_config['dropout_aug'] and self.imageset == 'train':
            ...
            ) * self.config['dataset_params']['max_dropout_ratio']
            drop_idx = np.where(np.random.random(
                (lidar2ego_xyz.shape[0])) <= dropout_ratio)[0]
        ...
        ego2lidar_xyz = geometry.ego_to_lidar_point(
            lidar2ego_xyz, data_dict['calib_info']['lidar_pose'])
        projected_points2d, _, inner_indices = geometry.projection(lidar_points=ego2lidar_xyz[:, :3],
                                                                   ...
                                                                   camera_intrinsics=data_dict[
                                                                       'calib_info']['camera_intrinsics'],
                                                                   filter_outliers=True)
        ...
        if self.resize:
            ...
            # scale image points
            points_img[:, 0] = float(
                self.resize[1]) / data_dict['img'].size[1] * np.floor(points_img[:, 0])
            points_img[:, 1] = float(
                self.resize[0]) / data_dict['img'].size[0] * np.floor(points_img[:, 1])

            # resize image
            data_dict['img'] = data_dict['img'].resize(
                self.resize, Image.BILINEAR)
        ...
        data_dict['img'] = np.array(
            data_dict['img'], dtype=np.float32, copy=False) / 255.

        if np.random.rand() < self.config['dataset_params']['flip2d']:
            data_dict['img'] = np.ascontiguousarray(data_dict['img'])
            img_indices[:, 1] = data_dict['img'].shape[1] - \
                1 - img_indices[:, 1]
