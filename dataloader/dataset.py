...
class point_image_dataset_semkitti(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels_ = labels_[keep_idx]
        img = cv2.imread('/share/qi.chao/open_sor_data/kitti/sequences/00/image_2/000000.png')
        veg = np.ones(labels_.shape[0], dtype=bool)
        # veg = np.logical_and(veg, labels_==16)
        veg_img = img_points[keep_idx_img_pts][veg]
        for i in range(len(veg_img)):
            cv2.circle(img, (int(veg_img[i][0]), int(veg_img[i][1])), 1, (0,255,0), 4)
        cv2.imwrite('work_dirs/kitti/a.jpg', img)
        """
