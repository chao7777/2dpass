...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0, mlp_ratio=4., qkv_bias=True, qk_scale=None,
        ...
        self.attn = WindowAttention(dim, to_2tuple(window_size), num_heads=num_heads, qkv_bias=qkv_bias,
...
class BasicLayer(nn.Module):
    ...
    """
    def __init__(self, dim, depth, num_head, window_size=7, mlp_ratio=4, qkv_bias=True, qk_scale=None,
                 drop=0.0, attn_drop=0.0, drop_path=0.0, norm_layer=nn.LayerNorm, downsample=None,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                dim=dim, num_heads=num_head, window_size=window_size, 
...
class SwinTransformer(nn.modules):
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2],
        ...
        self.patch_embed = PatchEmbed(patch_size, in_chans, embed_dim, norm_layer=norm_layer if patch_norm else None)

        # absolute position embedding
        if self.ape:
            path_row, path_col = pretrain_img_size // patch_size, pretrain_img_size // patch_size
            self.absolute_pos_embed = nn.Parameter(torch.zeros(1, embed_dim, path_row, path_col))
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                dim=int(embed_dim * 2 ** i_layer), depth=depths[i_layer], num_head=num_heads[i_layer],
