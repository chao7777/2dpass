...
class LiftSplatShoot(nn.Module):
    ...
    """
    def __init__(self, lss=False, final_dim=(900, 1600), camera_depth_range=[4.0, 45.0, 1.0], pc_range=[-50, -50, -5, 50, 50, 3],
                 downsample=4, grid=3, inputC=256, camC=64):
        super().__init__()
        ...
        self.fH, self.fW = self.final_dim[0] // downsample, self.final_dim[1] // downsample
        self.camC = camC
        self.inputC = inputC
        self.frustum = self.create_frustum()
        self.D, _, _ = self.frustum.shape
    ...
    def create_frustum(self):
        ...
        ds = torch.arange(*self.grid_conf['dbound'], dtype=torch.float).view(-1, 1, 1).expand(-1, self.fH, self.fW)
        D, _, _ = ds.shape
        xs = torch.linspace(0, ogfW - 1, self.fW, dtype=torch.float).view(1, 1, self.fW).expand(D, self.fH, self.fW)
        ys = torch.linspace(0, ogfH - 1, self.fH, dtype=torch.float).view(1, self.fH, 1).expand(D, self.fH, self.fW)
